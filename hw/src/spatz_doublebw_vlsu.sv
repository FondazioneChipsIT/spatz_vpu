...
`include "common_cells/registers.svh"
  ...
  // Memory side segment signals
  // Do we have a segmented memory access
  logic mem_is_segmented;
  assign mem_is_segmented = (mem_spatz_req.op == VLSEG) || (mem_spatz_req.op == VSSEG);

  // Number of segments (1-8). nf=0 --> mem_seg_nf=1 --> regular load/store
  logic [3:0] mem_seg_nf;
  assign mem_seg_nf = {1'b0, mem_spatz_req.nf} + 4'd1;

  // Shared memory and commit signals
  // segment counter --> it cycles from 0 to nf-1
  logic [2:0] seg_cnt_d, seg_cnt_q;
  `FF(seg_cnt_q, seg_cnt_d, 3'd0)

  // Flag: mem and commit sides complete current segment
  logic seg_completed;
...
`endif
  ...
  logic commit_is_segmented;
  assign commit_is_segmented = (commit_insn_q.op == VLSEG) || (commit_insn_q.op == VSSEG);

  // nf=0 --> commit_seg_nf=1 --> regular load/store
  logic [3:0] commit_seg_nf;
  assign commit_seg_nf = {1'b0, commit_insn_q.nf} + 4'd1;
...
//   0               32               64
  ...
  always_comb begin : gen_vreg_addr
    for (int intf = 0; intf < NrInterfaces; intf++) begin : gen_vreg_addr_intf
      if (commit_is_segmented)
        vd_vreg_addr[intf] = ((commit_insn_q.vd + {2'b00, seg_cnt_q}) << $clog2(NrWordsPerVector)) + $unsigned(vd_elem_id[intf]);
      else
  ...
  always_comb begin: control_proc
    ...
    // Finished the execution of current segment
    if (commit_insn_valid && &(commit_finished_q | (commit_finished_d & commit_counter_en)) && mem_insn_finished_q[commit_insn_q.id]) begin
      // check if there are further segments
      if (commit_is_segmented && seg_cnt_q < (commit_seg_nf[2:0] - 3'd1)) begin
        // intermediate field: advance to next field but doesn't complete instruction
        seg_cnt_d = seg_cnt_q + 1;
        seg_completed = 1'b1;
      end else begin
  ...
  // for segmented instructions this only fires on the last segment
  always_comb begin: mem_acc_finished
    if (commit_is_segmented)
      spatz_mem_finished_o = commit_insn_valid && &(commit_finished_q | (commit_finished_d & commit_counter_en)) && mem_insn_finished_q[commit_insn_q.id] && (seg_cnt_q == (commit_seg_nf[2:0] - 3'd1));
    else
...
`endif
  ...
  for (genvar intf = 0; intf < NrInterfaces; intf++) begin: gen_vreg_counter_proc_intf
    for (genvar fu = 0; fu < N_FU; fu++) begin: gen_vreg_counter_proc_intf_fu
      ...
      always_comb begin
        ...
        // reset commit counter for the next segment
        commit_counter_load[intf][fu] = commit_insn_pop || seg_completed;
  ...
  for (genvar intf = 0; intf < NrInterfaces; intf++) begin: gen_mem_counter_proc_intf
    for (genvar fu = 0; fu < N_FU; fu++) begin: gen_mem_counter_proc_intf_fu
      ...
      always_comb begin
        ...
        // reset mem counter for the next segment
        mem_counter_load[intf][fu]    = mem_spatz_req_ready || seg_completed;
  ...
  always_comb begin
    for (int intf = 0; intf < NrInterfaces; intf++) begin
      ...
      // response is valid only on the last segment commit
      if (commit_is_segmented)
        vrf_req_d[intf].rsp_valid = commit_insn_valid && &commit_finished_d[intf] && mem_insn_finished_d[commit_insn_q.id] && (seg_cnt_q == (commit_seg_nf[2:0] - 3'd1));
      else
