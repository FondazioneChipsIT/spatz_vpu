...
This document describes the standalone `spatz_vpu` package (top-level module `spatz`, file `hw/src/spatz.sv`) as seen from its boundary, with the level of detail needed to build a UVM verification environment around it.
Everything stated here is derived from the RTL in this repository; points that are observations about the current RTL behaviour (rather than intended specification) are explicitly marked as such in [Verification Notes](#verification-notes).
...
- [Verification Notes](#verification-notes)
  ...
  - [Suggested UVM Environment](#suggested-uvm-environment)
  - [Reference Model and Checking](#reference-model-and-checking)
  - [Functional Coverage](#functional-coverage)
  - [Suggested Assertions](#suggested-assertions)
  - [RTL Points of Attention](#rtl-points-of-attention)
...
The top-level parameters are **not** taken from `spatz_pkg`: they are set by the module that instantiates `spatz` (in the cluster, `spatz_cc`, fed by the wrapper generated from the hjson config). In a standalone testbench no wrapper exists, so the TB top has to set them itself; use the values below to match the reference cluster.
...
The *Default* column is the value resulting from that file, not the default of the `spatz` module declaration (which for `FPUImplementation` is `'0`, i.e. every FPnew unit disabled: a standalone TB must always pass it explicitly).
...
- The address type is chosen by the integrator (the cluster uses a narrow TCDM address); the VLSU computes 32-bit addresses internally, so a 32-bit `addr_t` is recommended for a standalone TB.
...
- There is no response `ready`: the memory model must never expect backpressure on responses. For loads the VLSU never has more than 8 requests per port outstanding or buffered (bounded by its offset queue), so its buffers cannot overflow. Stores are **not** limited by the VLSU; the per-port store-acknowledge counter is only 3 bits wide, so a memory model must not keep more than 7 stores per port unacknowledged (see [RTL Points of Attention](#rtl-points-of-attention)).
...
- Widening FP instructions widen the operands before the FPU with a simple exponent re-bias (`widen_fp*_to_fp*` in `spatz_pkg`), not through an FPnew conversion: denormals, infinities and NaNs of the narrow format are not converted per IEEE rules (see [RTL Points of Attention](#rtl-points-of-attention)).
...
The configuration used by the reference cluster (`spatz_cluster_wrapper.sv`), recommended for the standalone TB:
...
| Item | Visible at boundary | How to check |
|------|---------------------|--------------|
| CSR values, `vl` after `vsetvl*` | `rsp_o` | direct compare |
| Scalar integer results | `rsp_o` | direct compare |
| Scalar FP results / `vfmv.f.s` | no (internal FP RF) | move to integer (`fmv.x.w`) or store (`fsw/fsd`), or backdoor `i_fpu_sequencer.i_fpr` |
| Vector register contents | no | store to memory (`vse*`) or backdoor on `i_vrf.gen_reg_banks[b].gen_vrf_slice[fu].i_vregfile.mem` (latch array, see [VRF](#vector-register-file-vrf) for the bank mapping) |
| Memory side effects | VLSU ports, FP LSU | per-transaction monitor and/or final memory image |
| FP flags | `fpu_status_o` | accumulate (OR) over a test and compare with the reference `fflags` |
| Instruction completion | `spatz_mem_finished_o`, `issue_rsp_o.isfloat` | end-of-test detection: `isfloat == 0`, no memory transaction pending, write-back queue empty |

### Suggested UVM Environment

| Agent | Mode | Interface | Notes |
|-------|------|-----------|-------|
| `spatz_issue_agent` | active master | `issue_valid_i/ready_o/req_i/rsp_o` | Drives instruction streams (from a generator or from an ELF/trace), samples `issue_rsp_o` in the handshake cycle. Must honour ordering rules of the core (see below) |
| `spatz_wb_agent` | active slave | `rsp_valid_o/ready_i/rsp_o` | Random `rsp_ready_i` backpressure |
| `spatz_mem_agent` ×`NrMemPorts` | reactive slave | `spatz_mem_*[p]` | Shared backing memory model; random `ready` deassertion, random but **in-order** response latency, response for every write |
| `spatz_fplsu_agent` | reactive slave | `fp_lsu_mem_*` | `reqrsp` slave on the same memory model |
| `spatz_side_agent` | active | `fpu_rnd_mode_i`, `fpu_fmt_mode_i` | Must be stable across the handshake of an FP instruction (models `fcsr.frm`/fmode of the core); monitors `fpu_status_o` |
| `spatz_mem_finished_monitor` | passive | `spatz_mem_*finished_o` | Counts completed vector / FP memory instructions |

Ordering rules the issue agent should model (they are guaranteed by Snitch in the real system and are **not** enforced by Spatz):
- Scalar integer registers: the core does not issue an instruction reading an integer `rd` still pending in Spatz (Spatz forwards `data_arga/argb` as given).
- Scalar memory vs vector memory: Snitch holds its own loads/stores using `loadstore` and `spatz_mem_*finished_o` counters (stores wait for all Spatz memory ops, loads wait for Spatz stores). A TB driving raw instruction streams with a shared memory model must either follow the same rule or accept that memory ordering between scalar and vector accesses is undefined.
- `fcsr`: the core does not read `fflags/frm` while `isfloat = 1`.

### Reference Model and Checking

- Recommended golden model: an RVV-1.0 ISS (e.g. Spike with `--varch=vlen:512,elen:64`, as used by the SW flow) or a C/SystemVerilog RVV model through DPI, configured with the Spatz restrictions: tail-undisturbed always, mask-undisturbed (except comparisons with `vma = 1`), `vill` at bit 8 of `vtype`, unsupported instructions illegal, CSR `vxsat/vxrm/vcsr` reading 0.
- Because vector instructions produce no boundary transaction except memory traffic, the most robust scoreboard is **end-of-sequence**: compare the memory image and the set of `rsp_o` values (ordered, one per WB instruction) with the reference, plus accumulated `fflags`. Per-store comparison is also possible since stores of one instruction are issued in element order per port.
- Memory transactions of different vector instructions can overlap in time (the next instruction's requests start while the previous one commits). Load and store requests are not interleaved on the request channel, but acknowledgements of the last stores may still be in flight when the first loads of the next instruction are sent (in-order responses make this safe).
- Widening FP operations and FP reductions need special handling in the reference: widening uses exponent re-bias (not an IEEE conversion), and ordered reductions (`vfredosum`) are computed as a tree, so results may differ in the last bits from a sequential ISS.
- Uninitialised VRF registers are X after reset: tests must write every register before reading it, or the reference must treat them as don't-care.

### Functional Coverage

Suggested coverpoints (crossed where meaningful):
- Instruction × SEW (8/16/32/64) × LMUL (F4, F2, 1, 2, 4, 8) × `vm` × `vl` class (0, 1, < word, = word, not multiple of word, VLMAX) × `vstart` (0 / non-zero).
- Memory: addressing mode × EEW × base-address alignment (0..7) × stride sign/magnitude (0, SEW, non-multiple, negative) × index EEW × `nf`.
- `vsetvl*`: AVL > / = / < VLMAX, `rs1 = x0` with `rd != x0` and `rd = x0`, keep-vl with same/different ratio, every illegal `vtype`, `vma/vta` values.
- FP: rounding modes × format (incl. ALT via `fpu_fmt_mode_i`) × special operands (±0, ±inf, qNaN, sNaN, denormal) × each flag.
- Micro-architecture: number of in-flight instructions (1..`NrParallelInstructions`), chaining between each producer/consumer unit pair, WAR/WAW hazards, VRF bank conflicts (same bank, different ports), `BUF_FPU` FIFO full, IPU↔FPU switch, memory port backpressure length, response latency distribution, back-to-back load→store and store→load switches, `rsp_ready_i` backpressure during CSR bursts.
- Exceptions: each illegal-instruction class, instructions issued while `vill = 1`.

### Suggested Assertions

- Protocol: on every valid/ready interface, `valid && !ready |=> valid && $stable(payload)` (issue, rsp, each memory request, FP LSU).
- `spatz_mem_req_o[p].addr[2:0] == 0` and `amo == AMONone`.
- Memory model side: number of responses per port never exceeds the number of accepted requests; no more than 8 outstanding requests per port.
...
- After reset: `issue_rsp_o.isfloat == 0` and no memory request until the first memory instruction.
- Liveness: every accepted vector instruction eventually retires (`isfloat` falls within a bounded time once issue stops and memory responds).
- Internal (bind): FPU sequencer counters must not roll over (`MemoryOperationCounterRollover` already present), controller never pops the request buffer with no free ID.

### RTL Points of Attention

These are observations from reading the current RTL. They are listed as targets for directed tests; each should be confirmed in simulation before being treated as a bug.

1. **`vill` exception vs execution.** The controller reports `exception = 1` for vector instructions issued with `vill = 1`, but the dispatch condition (`spatz_req_valid`) only checks `instr_illegal` from the decoder, not `vill`: the instruction may still be executed with an illegal `vtype`.
2. **Illegal instruction held in the request buffer.** `spatz_req_illegal` is computed from the *current* decoder output, while the popped request comes from the 1-entry buffer. If an illegal instruction cannot be popped in its decode cycle (e.g. the response register is full, since an illegal instruction decodes with `ex_unit = CON`), it is popped later with `spatz_req_illegal = 0` and could be executed/retired (spurious write-back).
3. **Carry-in for `vadc`/`vsbc`/`vmadc.v*m`/`vmsbc.v*m`.** The VFU ties the IPU `carry_i` input to `'0`; check that `v0` carry/borrow is actually applied.
4. **`vta` ignored.** Tail is always undisturbed; a reference model with tail-agnostic "all ones" semantics will mismatch.
5. **Widening FP conversion.** `widen_fp*_to_fp*` re-bias the exponent without handling zero, denormals, infinities and NaNs; widening instructions with special operands are expected to differ from IEEE behaviour.
6. **Ordered FP reduction.** `vfredosum` is decoded like `vfredusum` (tree reduction).
7. **`vl`/`vtype` CSR writes.** `csrrw` to `vl`/`vtype`/`vlenb` is not illegal and silently ignored.
8. **`RegisterRsp` and `NumOutstandingLoads` for the VLSU** are not effective (see [Parameters](#parameters)).
9. **`fpu_status_o` is not qualified** by FPU output valid; it is sampled every cycle, so flags may be reported more than once or one cycle late. Only the accumulated value is meaningful.
10. **`simd_mask_i = '1`**: FP flags from inactive/tail lanes and from lanes beyond `vl` in the last word are not masked, which can produce flags the reference model does not produce.
11. **Default `FPUImplementation = '0`** disables all FPnew units: a TB forgetting to set it will hang on FP instructions (no `out_valid`).
12. **Outstanding stores.** `store_count_q` in the VLSU is `idx_width(8) = 3` bits and stores are not throttled by it: with a memory latency long enough to have 8 or more unacknowledged stores on one port the counter wraps and subsequent load data may be dropped or mis-assigned. Cover with long-latency memory configurations.
13. **Stride not multiple of the element size.** The strided address is computed as `byte_index * (rs2 >> vsew)`: the low bits of a stride that is not a multiple of `SEW/8` are lost. Negative strides that are multiples of `SEW/8` work (modulo-2^32 arithmetic).
